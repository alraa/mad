...
$(document).ready(function() {
    ...
    $('.slider_tab_list').on('click', 'a', function(e) {
        e.preventDefault();
        $('.slider_tab_item').removeClass('active');
        $(this).parent().addClass('active');
        var slideIndex = $(this).closest('li').attr('data-slick-index');
        $slidTab.slick('slickGoTo', parseInt(slideIndex));
    });


    // On before slide change
    $('.opp_slider').on('beforeChange', function(event, slick, currentSlide, nextSlide) {
        if (nextSlide >= 7) {
            $('.tab_cont').tabs("option", "active", 1);
        }

        if (nextSlide < 7) {
            $('.tab_cont').tabs("option", "active", 0);
        }


        $('.opp_cont').find('li[data-slick-index=' + currentSlide + ']').removeClass('active');
        $('.opp_cont').find('li[data-slick-index=' + nextSlide + ']').addClass('active');

        $('.slider_tab_box[data-slick-index=' + currentSlide + ']').hide();
        $('.slider_tab_box[data-slick-index=' + nextSlide + ']').fadeIn(200);
    });
    ...
    $('.review_slider').slick({
        ...
        arrows: true,
        fade: true,
        nextArrow: '<button type="button" class="slick-next">Показать еще отзыв</button>'
    });
    ...
    $('.tab_cont').tabs({
        ...
        activate: function(event, ui) {
            var active = $('.tab_cont').tabs('option', 'active');
            var cur_tab = $(".tab_cont ul>li a").eq(active).attr("href");
            if (cur_tab == '#tab-2') {
                $slidTab.slick('slickGoTo', 7);
                $('.opp_cont').find('li').removeClass('active');
                $('.opp_cont').find('li[data-slick-index=' + 7 + ']').addClass('active');
            }
            if (cur_tab == '#tab-1') {
                $slidTab.slick('slickGoTo', 0);
                $('.opp_cont').find('li').removeClass('active');
                $('.opp_cont').find('li[data-slick-index=' + 0 + ']').addClass('active');
            }
        }
    });
    ...
    $('.fancy_more').fancybox({
        ...
        fitToView: false,
        beforeShow: function() {
            var catalogList = $(this.element).parent().prev().clone();
            $('#compare_more_popup .compare_popup_wrap').append().html(catalogList);
        },
        tpl: {
            closeBtn: '<a title="Закрыть" class="fancybox-item fancybox-close close_gray" href="javascript:;"></a>'
        }
    });
    ...
    $('.form_popup').submit(function(e) {
        ...
        var form = $(this);
        var hasErrors = false;
        form.find('.input_text_req').each(function(index) {
            var required = $(this).attr('required');
            if (($.trim($(this).val()) == '')) {
                hasErrors = true;
                $(this).addClass('error').after('<p class="required_field">Поле необходимо заполнить</p>').after('<i class="attention_icon"></i>');
            } else {
                $(this).removeClass('error').nextAll().detach();
            }
        });
    });
    ...
    //Amount DC
    var a;
    var inputNum = $('.input_text_num');
    $('.button_input_incr').click(function() {
        a = inputNum.val();
        a++;
        inputNum.val(a);
    });
    $('.button_input_decr').click(function() {
        a = inputNum.val();
        if (a == 0) {
            inputNum.val(0);
        } else {
